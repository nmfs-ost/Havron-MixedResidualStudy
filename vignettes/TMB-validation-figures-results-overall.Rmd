...
df.err <- pvals %>% dplyr::filter(., version == "h0" & 
                          ...
                          (test == "Lilliefors" | test == "Anderson-Darling" |
                            ...
                            !(method == "Unconditional ecdf" & type == "GLMM") & 
...
df.corr %>% dplyr::filter(., type == "LMM" ) %>% 
  plot.err.pow(., misp.filter = c("Correct",  "Misp. RE Model"),  test.filter = 
                 c("Kolmogorov- Smirnov", "Autocorrelation") )
```
...
## Power by Sample summarize
```{r, ss-rw-mispre-glmm, echo = FALSE, message = FALSE, warning = FALSE, fig.height = 6, caption = "Power by Sample Size for the GLMM randomwalk model with mis-specified covariance structure."}
results.randomwalk.lmm$pval %>% plot_ss_pow_by_dim()
results.randomwalk.glmm$pval %>% plot_ss_pow_by_dim()
```
