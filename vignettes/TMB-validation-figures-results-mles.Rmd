...
## Phylo
### LMM
```{r, echo = FALSE, fig.cap = "Bias in parameter estimates for the phylogenetic linear mixed model when the model is correctly specified across 1000 simulations."}
mle.df <- dplyr::filter(mles, model == 'phylo' & type == 'LMM')
...
### GLMM
```{r, echo = FALSE, fig.cap = "Bias in parameter estimates for the phylogenetic model when the model is correctly specified across 1000 simulations."}
mle.df <- dplyr::filter(mles, model == 'phylo' & type == 'GLMM')
mle.df[mle.df$par=="beta",]$par <- "beta_1"
plot.mles(mle.df)
mean(mle.df[mle.df$par=="beta_1" & mle.df$misp == "misscov",]$mle)
```
